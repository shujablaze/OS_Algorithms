...
struct process {
    int id, burst, art,priority, tat, wt, rt, ct;
};
...
int main()
{
    ...
    for(int i = 0; i < num; ++i) {
        ...
        cout << "Priority,arrival time,burst for process: ";
        cin >>  temp->priority>>temp->art>>temp->burst ;
        temp->id = i + 1;
...
void displayresults(vector<process*>& readyqueue)
{
    ...
    cout<<"\nAVERAGES:";
    cout << "\nwaiting time:" << wtime / (float)readyqueue.size() << endl;
    cout << "completion time:" << ctime / (float)readyqueue.size() << endl;
    cout << "turn-around time:" << ttime / (float)readyqueue.size() << endl;
    cout << "response time:" << rtime / (float)readyqueue.size() << endl;
