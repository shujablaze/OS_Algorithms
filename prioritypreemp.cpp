...
struct process {
    int id, burst, art, tat, wt,priority, rt { -1 }, ct { 0 };
};
...
void makegant(process* curprocess, vector<struct gannt>& chart, int sys);
void displayresults(vector<process*>& readyqueue, vector<int>& burstimes, vector<struct gannt>& gannt);
...
process* getprocess(vector<process*>& list)
{
    ...
    for(int i = 0; i < list.size(); ++i) {
        if(list[i]->burst > 0 && list[i]->priority <= min->priority) {
            min = list[i];
        }
    }
    return min;
