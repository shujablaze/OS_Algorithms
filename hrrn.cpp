...
int main()
{
    ...
    for(int i = 0; i < num; ++i) {
        ...
        cout << "Enter burst and arrival time " << i + 1 << ": ";
...
float calc_respratio(process * proc,float waittime)
{
        return (proc->burst+waittime)/(float)proc->burst;
}
...
process * getnextprocess(vector<process*>& list,int& totburst,int num)
{
    ...
    while(i < list.size() && totburst >= list[i]->art)
    {
        
        if(calc_respratio(list[i],totburst-list[i]->art) > calc_respratio(max_p,totburst-max_p->art))
            {
                max_p = list[i];
                min_index=i;
            }
